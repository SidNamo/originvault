                ...
                >
                  ...
                  {sortedFiles.map((file) => {
                    ...
                    return <article data-public-select-key={key} className={`public-item ${selected ? "selected" : ""}`} key={file.id} onClick={(event) => handleCardClick(key, event)} onDoubleClick={() => setPreviewFile(file)} onContextMenu={(event) => showItemMenu(event, { type: "file", item: file })}>
                      {viewMode === "preview" ? <LazyFileThumbnail fileId={file.id} fileName={file.name} mimeType={file.mimeType} version={file.sha256} kind={file.kind === "image" ? "image" : file.kind === "video" ? "video" : file.kind === "pdf" ? "pdf" : "unsupported"} source="public" shareToken={token} fallback={Icon} /> : <Icon />}<span><strong>{file.name}</strong><small>{formatBytes(file.sizeBytes)}</small></span>
